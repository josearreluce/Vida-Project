...
def getRelatedConditions(G,symptom): #Get related conditions given a symptom
  if ( (type(symptom) is not Symptom) ): #Make sure we're actually using a symptom
    nosymptom()
  ...
  for i in neighbors:
    if (type(neighbors[i]) is Symptom): #Check the typing
...
def getRelatedSymptomsC(G,condition):
  if ( (type(condition) is not Condition) ): #Make sure we're actually given a condition
    nocondition()
  ...
  for i in neighbors:
    if (type(neighbors[i]) is Condition):
...
def getRelatedSymptomsS(G,symptom): #Get related symptoms given a symptom
  if ( (type(condition) is not Symptom) ): #Make sure we're actually given a condition
    nocondition()
  ...
  for i in neighbors:
    if (type(neighbors[i]) is Symptom):
      symptoms.append(neighbors[i])
